...
function init (client, d3) {
  ...
  var dotRadius = function(type) {
    var radius = chart().prevChartWidth > WIDTH_BIG_DOTS ? 8 : (chart().prevChartWidth < WIDTH_SMALL_DOTS ? 1 : 8);
    if (type === 'mbg') {
      radius *= 0;
    } else if (type === 'forecast') {
      radius = Math.min(3, radius - 2);
    } else if (type === 'rawbg') {
      radius = Math.min(2, radius - 2);
    }
  ...
  renderer.bubbleScale = function bubbleScale () {
    // a higher bubbleScale will produce smaller bubbles (it's not a radius like focusDotRadius)//injection/carbs dot
      return (chart().prevChartWidth < WIDTH_SMALL_DOTS ? 1 : (chart().prevChartWidth < WIDTH_BIG_DOTS ? 1 : 1)) * focusRangeAdjustment();
  };
